...
Vanuit het ingebouwde profiel kan een owner of superadmin een eigen profiel maken. Een concreet profiel uit een toegestane context kan naar een gekozen LearningSpace worden gekopieerd. De bron ligt daarbij vast; de gebruiker kiest alleen de doel-LearningSpace, herkenbaar aan shortlabel en huidig actief profiel. Elke gewone profiel- of centrale templatekopie krijgt een nieuwe identiteit, de gekozen LearningSpace als managementcontext en een gevalideerde configsnapshot, maar wordt nooit automatisch actief. Activeren gebeurt uitsluitend als aparte handeling via **Ander profiel kiezen**. Alleen de expliciete LearningSpace-flow **Sjablonen → Kopiëren en activeren** maakt de nieuwe concrete templatesnapshot meteen actief. Alleen owners en superadmins kunnen de naam van een bestaand custom profiel wijzigen.

De scanner leest deze configuratie in B3.3.2 nog niet: profiel-, template-, default- en kopieeracties starten geen synchronisatie en sync en indexering blijven exact de bestaande implementatie gebruiken. Latere fases voegen terminologie, resources en scannerregels toe zonder het interne portfolio-domein of de read-only bronprincipes te wijzigen.
