...
De webapp voert database-migrations bij de eerste databaseaanroep automatisch en onder lock uit. De actuele release verwacht `schema_migrations` tot en met `041_source_profile_name_uniqueness`; controleer deze versie na deployment. Bestaande foutmeldingen worden zonder verlies in de thread-, issue- en reportstructuur opgenomen; de per-report leerling-lifecyclevelden starten voor historische data op `NULL` en migration 033 laat meerdere afzonderlijke submissions van dezelfde leerling op hetzelfde issue toe. Migrations 034 en 035 leggen het bronprofielfundament en de managementcontext vast. Migration 036 maakt het appbrede standaardsjabloon en zet alleen nog rechtstreeks aan de technische built-in gekoppelde LearningSpaces om naar onafhankelijke concrete snapshots, zonder bronverbindingen of geïndexeerde metadata te wijzigen. Migration 037 voegt expliciet `owner_user_id` toe aan custom bronprofielen, backfillt deterministisch vanuit bestaande LearningSpace-owners met de compatibility-superadmin als veilige fallback en behoudt profielsnapshots en usage-koppelingen. Migration 038 voegt nullable archiefvelden toe aan concrete profielen en templates zonder bestaande rijen te backfillen of snapshots te wijzigen. Migration 039 voegt ondersteuning voor externe portfoliolinks toe en migration 040 voegt opslag voor generieke bronresources toe. Migration 041 dwingt genormaliseerde bronprofiel- en sjabloonnamen atomair af en stopt zonder gegevenswijziging wanneer bestaande namen conflicteren. Hints-metadata en oefeningsnotities blijven optioneel en de huidige beheerder wordt als compatibility-superadmin gemigreerd.
