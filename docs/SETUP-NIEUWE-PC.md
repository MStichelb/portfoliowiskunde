...
De webapp voert database-migrations bij de eerste databaseaanroep automatisch en onder lock uit. De actuele release verwacht `schema_migrations` tot en met `036_global_source_profile_templates`; controleer deze versie na deployment. Bestaande foutmeldingen worden zonder verlies in de thread-, issue- en reportstructuur opgenomen; de per-report leerling-lifecyclevelden starten voor historische data op `NULL` en migration 033 laat meerdere afzonderlijke submissions van dezelfde leerling op hetzelfde issue toe. Migrations 034 en 035 leggen het bronprofielfundament en de managementcontext vast. Migration 036 maakt het appbrede standaardsjabloon en zet alleen nog rechtstreeks aan de technische built-in gekoppelde LearningSpaces om naar onafhankelijke concrete snapshots, zonder bronverbindingen of geïndexeerde metadata te wijzigen. Hints-metadata en oefeningsnotities blijven optioneel en de huidige beheerder wordt als compatibility-superadmin gemigreerd.
