...
De webapp voert database-migrations bij de eerste databaseaanroep automatisch en onder lock uit. De actuele release verwacht `schema_migrations` tot en met `035_source_profile_management_context`; controleer deze versie na deployment. Bestaande foutmeldingen worden zonder verlies in de thread-, issue- en reportstructuur opgenomen; de per-report leerling-lifecyclevelden starten voor historische data op `NULL` en migration 033 laat meerdere afzonderlijke submissions van dezelfde leerling op hetzelfde issue toe. Migration 034 koppelt iedere bestaande LearningSpace aan het ingebouwde bronprofiel **Standaard portfolio** zonder bronverbindingen of geïndexeerde metadata te wijzigen. Migration 035 voegt alleen de optionele LearningSpace-managementcontext voor custom bronprofielen toe. Hints-metadata en oefeningsnotities blijven optioneel en de huidige beheerder wordt als compatibility-superadmin gemigreerd.
