...
Elke LearningSpace is gekoppeld aan precies één concreet [bronprofiel](./docs/source-profiles.md). Elk custom profiel heeft één expliciete eigenaar; de management-LearningSpace is alleen provenance. Nieuwe LearningSpaces krijgen een onafhankelijke snapshot van het actuele appbrede standaardsjabloon; het ingebouwde profiel is alleen een technische fallback. Kopiëren maakt altijd een nieuw profiel in eigendom van de huidige gebruiker, naar een LearningSpace waarvan die gebruiker owner is, en activeert de kopie daar meteen. Een editor ziet alleen foreign profielen die actueel gebruikt worden in een editor-LearningSpace; een pure editor zonder eigen owner-LearningSpace kan niet kopiëren. Alleen de profieleigenaar of superadmin kan een concreet profiel hernoemen of rechtstreeks koppelen, zodat meerdere LearningSpaces exact hetzelfde profiel delen. Templates blijven buiten het teacher-ownershipmodel en kunnen nooit rechtstreeks gekoppeld worden. Alleen superadmins muteren templates en het standaardsjabloon. De huidige scanner blijft ongewijzigd en gebruikt deze profielconfiguratie nog niet.
