...
Elke LearningSpace is gekoppeld aan precies één concreet [bronprofiel](./docs/source-profiles.md). De LearningSpace-instellingen tonen dit profiel read-only; kiezen, kopiëren, koppelen en hernoemen gebeuren uitsluitend centraal via **Beheer → Bronprofielen**, verdeeld over **Mijn bronprofielen**, **Bronprofielen uit leeromgevingen** en **Sjablonen**. Elk custom profiel heeft één expliciete eigenaar; de management-LearningSpace is alleen provenance. Kopiëren maakt een nieuw onafhankelijk profiel in eigendom van de huidige gebruiker en activeert het in een geldige owner-LearningSpace. Koppelen deelt hetzelfde profiel-ID en is alleen toegestaan wanneer de profieleigenaar ook owner van de doel-LearningSpace is; ook superadmins respecteren die ownergrens. Foreign editor-profielen zijn read-only en alleen kopieerbaar naar een eigen owner-LearningSpace. Templates blijven buiten het teacher-ownershipmodel, zijn nooit rechtstreeks koppelbaar en kunnen alleen door superadmins worden gemuteerd. De huidige scanner blijft ongewijzigd.
