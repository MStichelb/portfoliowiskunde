...
import { createErrorReport } from "@/lib/repositories";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as { exerciseId?: unknown; variant?: unknown; message?: unknown; website?: unknown } | null;
  if (!body || body.website) return Response.json({ ok: true });
  if (typeof body.exerciseId !== "string" || (body.variant !== "standard" && body.variant !== "alternative") || typeof body.message !== "string") return Response.json({ error: "Ongeldige melding." }, { status: 400 });
  ...
  try {
    await createErrorReport({ exerciseId: body.exerciseId, variant: body.variant, message: body.message, rateLimitKey });
