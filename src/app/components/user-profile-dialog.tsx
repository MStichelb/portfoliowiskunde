...
export function UserProfileDialogContent({ profile, titleId, closeButtonRef, onClose = () => undefined }: { profile: UserProfileData; titleId: string; closeButtonRef?: Ref<HTMLButtonElement>; onClose?: () => void }) {
  return <div className="user-profile-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
    ...
    <div className="user-profile-content">
      ...
      <ProfileList label="Groepen" empty={profile.groups.length === 0} emptyLabel="Geen opgeslagen Smartschoolgroepen"><NeutralBadges values={profile.groups} /></ProfileList>
      {profile.role === "teacher" ? <ProfileList label="Verbindingen" empty={profile.connections.length === 0} emptyLabel="Geen persoonlijke verbindingen"><ConnectionBadges connections={profile.connections} /></ProfileList> : null}
      {profile.role === "teacher" ? <ProfileList label="Eigenaar van" empty={profile.ownerSpaces.length === 0} emptyLabel="Geen leeromgevingen"><SpaceBadges spaces={profile.ownerSpaces} role="owner" /></ProfileList> : null}
      {profile.role === "teacher" ? <ProfileList label="Bewerker van" empty={profile.editorSpaces.length === 0} emptyLabel="Geen leeromgevingen"><SpaceBadges spaces={profile.editorSpaces} role="editor" /></ProfileList> : null}
...
function ProfileList({ label, empty, emptyLabel, children }: { label: string; empty: boolean; emptyLabel: string; children: React.ReactNode }) {
  return <div className="user-profile-field"><strong>{label}</strong>{empty ? <span className="muted-value">{emptyLabel}</span> : children}</div>;
}
