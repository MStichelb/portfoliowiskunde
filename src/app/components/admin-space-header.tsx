...
import { getLatestSyncSummary, getOpenErrorIssueCount, type LearningSpace } from "@/lib/repositories";

export async function AdminSpaceHeader({ current, section, user }: { current: LearningSpace; section: AdminSpaceSection; user: AppUser }) {
  const [sync, reportCount] = await Promise.all([getLatestSyncSummary(current.id), getOpenErrorIssueCount(current.id)]);
