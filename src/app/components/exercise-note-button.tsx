    ...
    {open ? <div className="confirm-backdrop" role="presentation">
      <div className="exercise-note-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
        ...
        <div className="exercise-note-dialog-actions">
          <div>{hasNote ? <ConfirmActionButton action={deleteExerciseNoteAction} fields={{ id: exerciseId }} className="danger-button" label={<><Trash2 size={16} aria-hidden />Notitie verwijderen</>} confirmTitle="Notitie verwijderen?" confirmText="Deze notitie wordt permanent verwijderd." /> : null}</div>
          <div><button className="secondary-button" type="button" onClick={close}>Annuleren</button><button className="primary-button" type="submit" form={formId}>Opslaan</button></div>
        </div>
      </div>
    </div> : null}
  </>;
