...
export function ErrorReportForm({ exerciseId, variants }: { exerciseId: string; variants: Array<"standard" | "alternative"> }) {
  ...
  return <form action={submit} className="report-form"><h2>Foutje gezien? Meld het</h2><label>Wat heb je opgemerkt?<textarea name="message" required minLength={3} maxLength={2000} /></label><label className="honeypot">Website<input name="website" tabIndex={-1} autoComplete="off" /></label>{variants.length > 1 ? <label>Uitwerking<select name="variant">{variants.map((variant) => <option key={variant} value={variant}>{variant === "standard" ? "Uitwerking" : "Alternatieve uitwerking"}</option>)}</select></label> : <input type="hidden" name="variant" value="standard" />}<button className="secondary-button" disabled={status === "sending"}>{status === "sending" ? "Versturen..." : "Melding versturen"}</button>{status === "error" && <p className="error-message" role="alert">De melding kon niet worden verstuurd. Probeer later opnieuw.</p>}</form>;
}
