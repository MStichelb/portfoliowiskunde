...
export function SiteNavigation({ spaces, adminSpaces = spaces, user, emergencyAccess = false }: { spaces: SiteNavigationSpace[]; adminSpaces?: SiteNavigationSpace[]; user: SiteNavigationUser | null; emergencyAccess?: boolean }) {
  ...
  return <><nav className={`site-nav${adminRoute ? " admin-site-nav" : ""}`} aria-label="Hoofdnavigatie">
    <div className="site-nav-primary">
      ...
      {showSpaceNavigation ? <><span className="site-nav-divider site-nav-space-divider" aria-hidden /><SpaceNavigation spaces={contextSpaces} current={current} adminRoute={adminRoute} /></> : null}
    </div>
    <div className="site-nav-account">
      <span className="site-nav-divider" aria-hidden /><span className="site-nav-identity" aria-label={`Aangemeld als ${user.firstName}, ${roleLabel(user.role)}`}><RoleIcon role={user.role} /><span className="identity-welcome">Welkom, </span><span className="identity-name">{user.firstName}</span><span className="identity-welcome">!</span></span>
...
function SpaceNavigation({ spaces, current, adminRoute }: { spaces: SiteNavigationSpace[]; current: SiteNavigationSpace | null; adminRoute: boolean }) {
  ...
  return <>
    ...
    <details className="site-nav-mobile-spaces"><summary aria-label={`Leeromgeving kiezen, huidig: ${label}`}>Leeromg.</summary><div>{spaces.map((space) => <Link key={space.slug} className={space.slug === current?.slug ? "site-nav-space-current" : ""} href={href(space)}>{space.name}</Link>)}</div></details>
  </>;
}
