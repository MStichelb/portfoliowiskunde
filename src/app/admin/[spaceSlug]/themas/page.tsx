...
import { getAdminLearningSpaceBySlug, getLearningSpaces, getThemes } from "@/lib/repositories";
...
export default async function ThemesPage({ params }: { params: Promise<{ spaceSlug: string }> }) {
  ...
  const [space, spaces] = await Promise.all([getAdminLearningSpaceBySlug(spaceSlug), getLearningSpaces(true)]);
  if (!space) notFound();
