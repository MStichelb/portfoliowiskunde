...
import { getActiveWarningCounts, getAdminPortfolios, getLatestSyncSummary, getLearningSpaceBySlug, getLearningSpaces, getMissingIndexCounts, getOpenErrorReportCount, getThemes } from "@/lib/repositories";
...
export default async function LearningSpaceAdminPage({ params }: { params: Promise<{ spaceSlug: string }> }) {
  ...
  if (!space) notFound();
  const [spaces, portfolios, themes, warnings, reports, sync, missing] = await Promise.all([getLearningSpaces(true), getAdminPortfolios(space.id), getThemes(space.id), getActiveWarningCounts(space.id), getOpenErrorReportCount(space.id), getLatestSyncSummary(space.id), getMissingIndexCounts(space.id)]);
