...
import { savePortfolioAction, toggleSectionVisibilityAction } from "../../actions";
...
export default async function PortfolioAdminPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="page-shell portfolio-admin-page"><Link href="/admin" className="back-link">Terug naar overzicht</Link><header className="portfolio-detail-heading"><p className="eyebrow">Portfolio {portfolio.code}</p><h1>{portfolio.title}</h1><p className="file-reference">Gedetecteerde titel: {portfolio.detectedTitle}</p></header>
    ...
    <section className="admin-card" aria-labelledby="sections-title"><div className="card-heading"><div><h2 id="sections-title">Onderdelen</h2><p>Een zichtbaar onderdeel wacht automatisch wanneer het portfolio nog niet zichtbaar is.</p></div></div><div className="section-card-list">{portfolio.sections.map((section) => <details className="section-settings-card" key={section.id}><summary className="section-card-title"><span><strong>{section.order}. {section.title}</strong><small>{section.exercises.length} oefeningen</small></span><PublicationStatus status={section.effectiveStatus} /></summary><form action={toggleSectionVisibilityAction} className="section-settings-form"><input type="hidden" name="id" value={section.id} /><input type="hidden" name="portfolioId" value={portfolio.id} /><input type="hidden" name="visible" value={String(section.visibilityMode === "hidden")} /><p>Eigen status: <strong>{section.visibilityMode === "visible" ? "Zichtbaar" : "Verborgen"}</strong></p><SubmitButton className="secondary-button" pendingLabel="Bijwerken...">{section.visibilityMode === "visible" ? "Onderdeel verbergen" : "Onderdeel zichtbaar maken"}</SubmitButton></form></details>)}</div></section>
